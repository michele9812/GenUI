...
- **Implemented mobile viewport height optimization**: Created dynamic viewport filling system for onboarding cards using min(90vh, 600px/680px) heights with flexbox layouts instead of absolute positioning, ensuring perfect vertical centering across all mobile devices (iPhone SE to iPhone 16+) while maintaining consistent card dimensions between onboarding steps
...
- **Implemented fluid animations and transitions**: Created comprehensive animation system with cubic-bezier easing (0.4, 0, 0.2, 1) and variable durations (0.2s-0.6s) - enhanced buttons with hover scale effects, cards with entrance animations, and inputs with focus scaling for smooth user interactions
- **Standardized UI component radius consistency**: Updated all shadcn/ui base components (Button, Input, Select) to use responsive radius system instead of fixed rounded-md, ensuring consistent 6px-8px mobile, 8px-12px tablet, 12px-16px desktop border radius across all form elements
- **Optimized component transitions**: Replaced standard transition classes with fluid cubic-bezier animations, added scale transforms on hover/focus states, and implemented entrance animations (fadeInUp, scaleIn) for enhanced visual feedback and professional UI feel
- **Code cleanup and performance optimization**: Removed redundant CSS classes, eliminated duplicate radius specifications, cleaned up component comments, and consolidated animation/transition logic into centralized CSS classes for better maintainability
...
- **Implemented BackgroundGradientAnimation component**: Created animated gradient backgrounds for journey selection pages using persona-specific colors without affecting other components, featuring smooth CSS animations with Tailwind keyframes (moveVertical, moveInCircle, moveHorizontal) and persona-adaptive color schemes from background-colors utility
- **Standardized 24px gap spacing across all UI components**: Updated gap spacing in Button component (gap-2 → gap-6), MacOSDock hover labels (mb-2 → mb-6 with 24px bottom offset), Family Tabs (gap-1/gap-2 → gap-6), Senior Accordion (gap-6), Carousel3D (24px spacing for all breakpoints), and AI Prompt Box mobile version (gap-2 → gap-6) for consistent visual hierarchy and improved accessibility
- **Fixed floating elements positioning**: Anchored both MacOSDock in welcome page and PromptInputBox in journey-detail page to fixed position bottom-8 (32px from viewport bottom) with z-40 layering and full-width responsive padding for consistent floating navigation behavior across all pages
- **Optimized dock container spacing**: Reduced dock container height from 40px to 25px for hovered icons and adjusted hover label positioning to 16px gap from icon for optimal visual proximity and reduced viewport usage
- **Optimized accordion responsive spacing**: Updated Senior accordion to use 12px gap on mobile (gap-3) and 16px gap on desktop (md:gap-4) for better mobile viewport utilization while maintaining desktop readability
- **Reduced accordion content gap**: Decreased internal spacing within expanded accordion content from 24px (gap-6) to 8px (gap-2) between text and action button for more compact layout
- **Applied persona-specific colors to floating input**: Updated PromptInputBox attachment and send buttons to use persona accent colors with proper hover states - attachment button uses 50% opacity accent color, send button uses full accent color when active, maintaining red for recording state
- **Synchronized floating input icon colors**: Updated all button icons (ArrowUp, Mic, Square, StopCircle) to use persona accent colors dynamically instead of hardcoded colors, ensuring complete visual consistency with persona theming
- **Enhanced mobile floating input theming**: Updated mobile version of PromptInputBox to use persona accent colors for attachment and send buttons with proper hover states and fallback colors for consistency across desktop and mobile versions
- **Updated floating input default colors**: Changed icon buttons in floating input to use full primary color (100% opacity) in default state instead of 50% opacity, with hover effects using primary 90% opacity color and 10% opacity background circle for enhanced visual feedback
- **Added AirBuddy logo navigation**: Logo in journey-detail page now redirects to onboarding page (/) when clicked, while welcome page logo remains decorative without navigation functionality
- **Fixed iPhone SE carousel breakpoints**: Corrected mobile breakpoints for iPhone SE (≤320px) with optimized card dimensions (200px width/height), reduced spacing (16px), and minimal padding (4px) for proper fit within the 320px screen width
- **Resolved iPhone SE carousel-input overlap**: Reduced header spacing from 64px to 48px, decreased margins to 4px, lowered carousel container height to 240px, and reduced paddingTop to 8px to prevent floating input overlap on iPhone SE viewport
- **Ensured full width text containers**: Added w-full class, width: '100%' style, display: 'block', and boxSizing: 'border-box' to all text elements (H1, paragraphs) in journey-detail header section to force full width behavior and prevent content hugging on mobile
- **Fixed text overflow issues**: Capped all transform scale values at 1.1 maximum to prevent text overflow, added transformOrigin: 'center top' for proper scaling anchor, applied full width properties to welcome page text elements, and limited Senior accordion calc() font scaling to 1.15 maximum for viewport compatibility
- **Reduced mobile vertical spacing**: Decreased header margins (48px mobile vs 64px desktop), consistent 8px title spacing, reduced carousel container gaps (8px mobile vs 16px desktop), and optimized welcome page margins (32px iPhone SE, 40px mobile, 64px desktop) to prevent content overlapping on mobile viewports
- **Enhanced onboarding persona selection UX**: Added neutral gray hover state (hover:bg-gray-50) to select dropdown while restoring persona-specific styling to preview card - background uses primary color at 20% opacity, borders at 40% opacity, icons use full primary color, and typography applies persona-specific fonts with controlled scaling (max 1.1) for proper visual feedback during selection
- **Neutralized login page colors**: Replaced purple/blue gradient background with neutral gray-200, changed button from blue-indigo gradient to neutral gray-600/gray-700 hover states, and updated pilot icon container to gray-50 background for complete neutral appearance during onboarding
- **Fixed preview card layout and transitions**: Removed problematic scaling transforms, increased container height to 128px for proper text display, implemented instant transitions (duration-0) with key-based re-rendering, added proper text truncation (line-clamp-3) and min-width-0 for flex text container to prevent width overflow issues especially for Senior Globetrotter persona
- **Complete onboarding neutralization**: Removed all remaining purple/blue gradient backgrounds from onboarding.tsx, replaced with neutral gray-200 background, updated button from blue-indigo gradient to gray-600/gray-700 states, and changed pilot icon container to gray-50 background for consistent neutral appearance across both onboarding steps
- **Neutralized CSS custom properties**: Fixed persistent purple in select dropdown by updating root CSS variables --primary from blue (hsl(207, 90%, 54%)) to neutral gray (hsl(0, 0%, 40%)) and --ring to gray (hsl(0, 0%, 60%)) to ensure all focus rings and primary color references use neutral gray instead of blue/purple colors
- **Fixed select item focus colors**: Replaced focus:bg-accent with focus:bg-gray-100 in SelectItem component and neutralized --accent CSS variables to ensure select dropdown items use gray colors instead of purple/blue when focused or selected
- **Code cleanup and optimization**: Removed excessive empty lines, eliminated redundant comments, simplified component structures, removed unused Material Design icon imports, and optimized persona icon mapping for better maintainability and cleaner codebase
- **Implemented navbar component in welcome page**: Created full-width navbar with 8px padding, horizontal autolayout, left-aligned content, and relative positioning as requested - migrated AirBuddy logo and back button from absolute positioning to navbar structure
- **Unified header margins**: Standardized header section marginTop to 16px across all breakpoints (iPhone SE, mobile, desktop) for consistent spacing after navbar integration
- **Applied navbar to all customer journey pages**: Implemented systematic navbar with 8px padding and horizontal layout across onboarding.tsx, login.tsx, journey-detail.tsx, and welcome.tsx - updated journey-detail back button text to "Back to phase selection" and added proper routing links
- **Enhanced onboarding button with cyan gradient**: Updated Continue button from gray to cyan gradient (from-cyan-500 to-cyan-600) with darker hover state (from-cyan-600 to-cyan-700) for modern technological appearance
- **Applied cyan gradient to all onboarding buttons**: Extended cyan gradient styling to "Start the journey" button in login.tsx with disabled state styling (disabled:from-gray-400 disabled:to-gray-500) for consistent onboarding experience
- **Removed navbar from onboarding section**: Eliminated navbar component from onboarding.tsx to maintain clean, focused card-based layout during persona selection process
- **Standardized top-left navigation animations**: Unified microanimations for all logo and back buttons positioned in top-left area - consistent entrance animation (opacity 0→1, x -24→0, scale 0.9→1), 0.4s duration with cubic-bezier easing [0.16, 1, 0.3, 1], staggered delays (logo 0.1s, back button 0.2s), and uniform hover/tap effects (hover scale 1.02, tap scale 0.98)
...
- **Implemented fixed preview area heights to prevent card resizing**: Created fixed-height container wrapper (120px-150px across breakpoints) for persona preview in login step, ensuring no card resizing occurs during persona selection. Added matching spacer in onboarding step to maintain identical card dimensions. Updated overall card heights to 500px-620px to accommodate the fixed preview areas while maintaining proper button anchoring
- **Standardized 24px vertical spacing across all onboarding elements**: Applied systematic `space-y-6` (24px) gaps between all major components in both onboarding steps - header sections, form fields, preview areas, and content blocks. Restructured header areas to use individual divs with consistent spacing, ensuring uniform visual hierarchy and proper button anchoring at exactly 24px from card bottom
- **Fixed button anchoring at exactly 24px from bottom edge**: Moved padding from CardContent to button container using `pb-6` (24px) on button wrapper div with `mt-auto`, ensuring button is precisely positioned 24px from card bottom edge in both onboarding steps. Removed CardContent bottom padding to prevent double spacing
- **Implemented absolute positioning for fixed button placement**: Converted button containers to use `absolute bottom-6 left-6 right-6` positioning with CardContent as `relative` container, ensuring button is precisely fixed at 24px from card bottom edge regardless of content changes. Added `pb-20` to content areas to prevent overlap with fixed button positioning
- **Unified layout structure across onboarding steps**: Standardized both steps to use identical layout hierarchy - single content wrapper div with `h-full flex flex-col px-6 pt-8 pb-20`, followed by separate header and flexible content areas, ensuring consistent button positioning and content flow between steps
